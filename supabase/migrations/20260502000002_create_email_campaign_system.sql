...
CREATE POLICY "Admin can manage email templates" ON email_templates FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM users
...
CREATE POLICY "Admin can manage email campaigns" ON email_campaigns FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM users
...
CREATE POLICY "Admin can manage campaign emails" ON campaign_emails FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM users
...
CREATE POLICY "Admin can view all email logs" ON email_logs FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM users
...
CREATE POLICY "Admin can manage email logs" ON email_logs FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM users
...
CREATE POLICY "Users can view own campaign progress" ON user_campaign_progress FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Admin can manage campaign progress" ON user_campaign_progress;
CREATE POLICY "Admin can manage campaign progress" ON user_campaign_progress FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM users
