...
OUTPUT: Generate THREE complete, production-ready React component files that import existing components and arrange them into the specified layouts. Create thin wrapper components only where styling variations are needed.

---

## CONCEPT A — "The Job Site" (Bold, Direct, No-Nonsense)

Generate `HomeConceptA.jsx` with components arranged in this order:
...
LOST JOBS CALCULATOR (flagship):
- Use existing <LostRevenueCalculator /> component
- Wrap in a section with .ds-section class
- Background: --off-white (#F8FAFC)

Use .ds-reveal on every section for scroll animations. Add IntersectionObserver setup in useEffect.

---

## CONCEPT B — "The Dashboard" (Product-First, Data-Driven)

Generate `HomeConceptB.jsx` with components arranged in this order:
...
- Add subtle float animation: .ds-float class
- Full-width section with dark background

Use .ds-reveal on every section. Add IntersectionObserver.

---

## CONCEPT C — "The Testimonial" (Trust-First, Warm, Peer-Proof)

Generate `HomeConceptC.jsx` with components arranged in this order:
...
Generate all three files now. Each must be a complete, import-ready JSX component that follows the existing codebase patterns.