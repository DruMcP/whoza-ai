 ...
 */
async function sendEmail({ from, to, subject, html }: SendEmailParams): Promise<SendResult> {
  if (!RESEND_API_KEY) {
 ...
 */
function getScoreBandSummary(score: number): ScoreBandSummary {
  if (score <= 25) {
    return {
      band: 'Critical',
...
interface CompetitorAnalysisEmailParams {
  businessName: string
  score: number
  tradeType: string
  location: string
...
function generateCompetitorAnalysisEmailTemplate({
  ...
  websiteUrl,
  summaryText
}: CompetitorAnalysisEmailParams): string {
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #0f172a; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3);">
          ...
          <tr>
            <td style="padding: 0 30px 30px; text-align: center;">
              <a href="${process.env.NEXT_PUBLIC_SITE_URL || 'https://whoza.ai'}/pricing" style="display: inline-block; background: linear-gradient(135deg, #4ade80 0%, #22c55e 100%); color: #0f172a; text-decoration: none; font-size: 16px; font-weight: 700; padding: 16px 32px; border-radius: 8px; box-shadow: 0 4px 12px rgba(74, 222, 128, 0.3);">
...
interface SendCompetitorAnalysisParams {
  email: string
  businessName: string
  score: number
  tradeType: string
...
export async function sendCompetitorAnalysisResultsEmail({
  ...
  websiteUrl,
  summaryText
}: SendCompetitorAnalysisParams): Promise<SendResult> {
  try {
