...
export async function findBusinessViaPlaces(businessName: string, locationHint = ''): Promise<BusinessPlace | null> {
  try {
...
function inferTradeFromTypes(types: string[], name: string): string {
...
export async function findCompetitorViaAI(trade: string, location: string): Promise<CompetitorResult> {
...
export function generateBlindSpotReasons(
  business: BusinessPlace,
  competitor: BusinessPlace,
  trade: string,
  location: string
): BlindSpotReason[] {
...
export function generateQuickFixes(trade: string, location: string): QuickFix[] {
  return [
    {
...
export async function saveCompetitorAnalysis(analysisData: any): Promise<any> {
...
export async function updateCompetitorAnalysis(id: string, updates: any): Promise<any> {
  const { data, error } = await supabase
    .from('competitor_analysis')
    .update(updates)
    .eq('id', id)
